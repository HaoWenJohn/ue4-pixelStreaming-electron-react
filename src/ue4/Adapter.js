...
function setOverlay(htmlClass, htmlElement, onClickFunction) {
  ...
  if (!videoPlayOverlay) {
    let playerDiv = document.getElementById('ue4');
...
function createWebRtcOffer() {
  if (webRtcPlayerObj) {
    ...
    showTextOverlay('开始加载模型，请稍后');
    webRtcPlayerObj.createOffer();
  } else {
    console.log('WebRTC player not setup, cannot create offer');
    showTextOverlay('模型加载出错');
  }
}
//webrtc加载较慢,使用缓存保存其加载完成前就发出的事件。
let cache=[];
function sendInputData(data) {
  cache.push(data);
  if (webRtcPlayerObj && cache.length>0) {
    while (cache.length>0){
      webRtcPlayerObj.send(cache.shift());
    }
    //resetAfkWarningTimer();
  }
}
...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  webRtcPlayerObj.onVideoInitialised = function() {
    if (ws && ws.readyState === WS_OPEN_STATE) {
      showTextOverlay('test!');
      emitUIInteraction({ 'Test': 'ok' });
      if (webRtcPlayerObj)
        webRtcPlayerObj.video.play();
      resizePlayerStyle();
...
function resizePlayerStyleToFillWindow(playerElement) {
  ...
  if (isNaN(videoAspectRatio)) {
    ...
    styleHeight = Math.floor(styleWidth * videoAspectRatio);
    styleTop = Math.floor((window.innerHeight - styleHeight) * 0.5);
    styleLeft = 0;


    //Video is now 100% of the playerElement, so set the playerElement style
    playerElement.style = 'top: ' + styleTop + 'px; left: ' + styleLeft + 'px; width: 100% ; height: ' + styleHeight + 'px; cursor: ' + styleCursor + '; ' + styleAdditional;
  }
}
...
function resizePlayerStyle(event) {
  let playerElement = document.getElementById('ue4');
  ...
  // let videoElements = playerElement.getElementsByTagName("video");
  // let videoElement;
  ...
  // let windowSmallerThanPlayer = window.innerWidth < videoElement.videoWidth || window.innerHeight < videoElement.videoHeight;
...
//未启用，若使用，请在ue4项目启动参数添加 -AllowPixelStreamingCommands
...
export { load, addResponseEventListener,removeResponseEventListener,emitUIInteraction};
